...
#done
#
#echo "Finally done with the loop!!!"
#sed -i "" -e"s/num_users=500/num_users=${LOAD_SCALE}/g" /faban/usersetup.properties
#echo $JAVA_HOME
#/faban/master/bin/startup.sh
#cd /web20_benchmark/build && java -jar Usergen.jar http://${WEB_SERVER_IP}:8080
#sed -i "" "s/<fa:scale.*/<fa:scale>${LOAD_SCALE}<\\/fa:scale>/g" /web20_benchmark/deploy/run.xml
#sed -i "" "s/<fa:rampUp.*/<fa:rampUp>10<\\/fa:rampUp>/g" /web20_benchmark/deploy/run.xml
#sed -i "" "s/<fa:rampDown.*/<fa:rampDown>10<\\/fa:rampDown>/g" /web20_benchmark/deploy/run.xml
#sed -i "" "s/<fa:steadyState.*/<fa:steadyState>30<\\/fa:steadyState>/g" /web20_benchmark/deploy/run.xml
#sed -i "" "s/<host.*/<host>${WEB_SERVER_IP}<\\/host>/g" /web20_benchmark/deploy/run.xml
#sed -i "" "s/<port.*/<port>8080<\\/port>/g" /web20_benchmark/deploy/run.xml
#sed -i "" "s/<outputDir.*/<outputDir>\/faban\/output<\\/outputDir>/g" /web20_benchmark/deploy/run.xml
#cd /web20_benchmark && ant run
